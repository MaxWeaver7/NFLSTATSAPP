...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        # API endpoints
        if path == "/api/teams":
            season = q_int("season") or 2025
            if sb is not None:
                standings = queries_supabase.get_team_standings(sb, season=season)
                self._json({"teams": standings})
            else:
                self._json({"error": "Supabase not connected"}, code=500)
            return
        ...
        if path == "/api/options":
            if sb is not None:
                ...
                # ... (omitted)
                pass # Original logic was longer, I'll match context below
            ...
            if sb is not None:
                ...
                st = sb.select(
                    ...
                    select=(
                        ...
                        "passing_attempts,passing_completions,passing_yards,passing_touchdowns,passing_interceptions,qbr"
                    ),
                ...
                adv_recv = sb.select(
                    ...
                    select="season,week,targets,receptions,yards,avg_yac,avg_expected_yac,avg_yac_above_expectation,avg_cushion,avg_separation,catch_percentage,percent_share_of_intended_air_yards",
                    filters=adv_filters,
                    order="week.asc",
                    limit=300,
                )
                ...
                adv_pass = sb.select(
                    ...
                    select="season,week,attempts,completions,pass_yards,pass_touchdowns,interceptions,passer_rating,completion_percentage_above_expectation,expected_completion_percentage,avg_time_to_throw,avg_intended_air_yards,avg_completed_air_yards,avg_air_distance,avg_air_yards_differential,avg_air_yards_to_sticks,aggressiveness",
                    filters=adv_filters,
                    order="week.asc",
                    limit=300,
                )
                ...
                # Restructure advanced stats into season totals + per-week game logs
                pass_season, pass_weekly = queries_supabase._split_season_weekly(adv_pass)
                rush_season, rush_weekly = queries_supabase._split_season_weekly(adv_rush)
                recv_season, recv_weekly = queries_supabase._split_season_weekly(adv_recv)

                season_advanced: dict[str, Any] = {}
                if pass_season:
                    season_advanced["passing"] = pass_season
                if rush_season:
                    season_advanced["rushing"] = rush_season
                if recv_season:
                    season_advanced["receiving"] = recv_season

                # Merge weekly advanced stats keyed by week number
                adv_game_logs: dict[str, dict[str, Any]] = {}
                all_weeks = sorted(set(list(pass_weekly) + list(rush_weekly) + list(recv_weekly)))
                for w in all_weeks:
                    entry: dict[str, Any] = {}
                    if w in pass_weekly:
                        entry["passing"] = pass_weekly[w]
                    if w in rush_weekly:
                        entry["rushing"] = rush_weekly[w]
                    if w in recv_weekly:
                        entry["receiving"] = recv_weekly[w]
                    adv_game_logs[str(w)] = entry

                # Position rankings
                position = r.get("position_abbreviation", "")
                rankings = queries_supabase.get_position_rankings(
                    sb, pid_int, season, position,
                    adv_recv, adv_rush, adv_pass,
                )

                # Snap history
                snap_history = queries_supabase.get_player_snap_history(
                    sb, player_name=name, team_abbr=team_abbr or "", season=season,
                )
                ...
                self._json(
                    {
                        "player": player,
                        "gameLogs": game_logs,
                        "seasonAdvanced": season_advanced,
                        "rankings": rankings,
                        "snapHistory": snap_history,
                        "advancedGameLogs": adv_game_logs,
                        "goatAdvanced": {
        ...
        if path == "/api/receiving_season":
            ...
            if sb is not None:
                ...
                if season is None:
                    ...
                    return
                rows = queries_supabase.receiving_season(
                    sb, 
                    season=season, 
                    team=q_str("team"), 
                    q=q_str("q"), 
        ...
        # Injuries endpoint - returns CURRENT injury statuses (deduplicated by player, latest only)
        if path == "/api/injuries":
            ...
            try:
                # Get max 2000 injuries to ensure we cover all active players
                # Ordered by date descending so we process newest first
                injuries = sb.select(
                    "nfl_injuries",
                    select="player_id,status,comment,date",
                    order="date.desc",
                    limit=2000,
                )
                
                # Deduplicate: Keep only the FIRST record seen for each player_id (which is the latest due to sort)
                injury_map = {}
                for inj in injuries:
                ...
                self._json({"injuries": injury_map})
            except Exception as e:
                self._json({"error": str(e)}, code=500)
            return
        ...
        if path == "/api/advanced/passing/season":
            ...
            rows = queries_supabase.advanced_passing_leaderboard(
                sb, season=season, sort_by=sort_by, position=position, team_abbr=team, q=q, limit=limit
            )
            self._json({"rows": rows})
            return
        ...
        if path == "/api/teams/standings":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            season = q_int("season") or 2025
            data = queries_supabase.get_team_standings(sb, season=season)
            self._json({"rows": data})
            return
        ...
        if path == "/api/team/roster":
            ...
            if not team:
                ...
                return
            season = q_int("season") or 2025
            data = queries_supabase.get_team_roster(sb, team_abbr=team, season=season)
            self._json({"players": data})
            return

        if path == "/api/team/season-stats":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            team = q_str("team")
            if not team:
                ...
                return
            season = q_int("season") or 2025
            season_type = q_int("season_type") or 2
            data = queries_supabase.get_team_season_stats(
                sb, team_abbr=team, season=season, season_type=season_type
            )
            self._json({"row": data})
            return

        if path == "/api/team/snaps":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            team = q_str("team")
            if not team:
                ...
                return
            season = q_int("season") or 2025
            data = queries_supabase.get_team_snaps(sb, team_abbr=team, season=season)
            self._json({"rows": data})
            return

        if path == "/api/team/leaders":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            team = q_str("team")
            if not team:
                ...
                return
            season = q_int("season") or 2025
            data = queries_supabase.get_team_leaders(sb, team_abbr=team, season=season)
            self._json({"row": data})
            return

        if path == "/api/teams/schedule":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            season = q_int("season") or 2025
            ...
            if not team:
                self._json({"error": "missing team"}, code=400)
                return
            data = queries_supabase.get_team_schedule(sb, team_abbr=team, season=season)
            self._json({"rows": data})
            return
        ...
        if path == "/api/schedule":
            ...
            team = q_str("team")
            if not team:
                rows = queries_supabase.get_weekly_schedule(sb, season=season, week=q_int("week") or 1)
                self._json({"rows": rows})
            else:
                rows = queries_supabase.get_team_schedule(sb, team_abbr=team, season=season)
                self._json({"rows": rows})
            return

        if path == "/api/matchup_history":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            team_a = q_str("team_a")
            team_b = q_str("team_b")
            limit = q_int("limit") or 5
            if not team_a or not team_b:
                self._json({"error": "missing team_a or team_b"}, code=400)
                return
            rows = queries_supabase.get_matchup_history(sb, team_a=team_a, team_b=team_b, limit=limit)
            self._json({"rows": rows})
            return


        if path == "/api/latest-week":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            season = q_int("season") or 2025
            week = queries_supabase.get_latest_week(sb, season=season)
            self._json({"week": week})
            return
        ...
        if path.startswith("/api/game/"):
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            game_id = path.split("/api/game/", 1)[1].strip()
            if not game_id:
                self._json({"error": "missing game_id"}, code=400)
                return
            data = queries_supabase.get_game_detail(sb, nflverse_game_id=game_id)
            if "error" in data and len(data) == 1:
                self._json(data, code=404)
            else:
                self._json(data)
            return

        if path == "/api/playoffs":
            if sb is None:
                self._json({"error": "Supabase not configured"}, code=503)
                return
            season = q_int("season") or 2025
            data = queries_supabase.get_playoff_games(sb, season=season)
            self._json({"rows": data})
            return
