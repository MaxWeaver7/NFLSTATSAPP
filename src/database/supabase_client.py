...
class SupabaseClient:
    ...
    def update(self, table: str, data: dict[str, Any], filters: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        # We want return=representation to get back what we updated
        ...
        if not (200 <= resp.status_code < 300):
            raise SupabaseError(f"Update failed table={table} status={resp.status_code} body={resp.text[:500]}")
            
        data = resp.json()
        if not isinstance(data, list):
            # Sometimes empty list if no match
            return []
        return data
    ...
    def delete(self, table: str, *, filters: dict[str, Any]) -> int:
        ...
        if not filters:
            raise SupabaseError("Delete requires filters to avoid accidental truncate")
            
        params: dict[str, Any] = {}
        params.update(filters)
        
        # HEAD request to get count? No, just delete.
        # But we want to return count. 
        # Prefer: count=exact? Or return=representation
        ...
        if not (200 <= resp.status_code < 300):
            raise SupabaseError(f"Delete failed table={table} status={resp.status_code} body={resp.text[:500]}")
            
        cr = resp.headers.get("Content-Range") or ""
        # Content-Range: */5 (where 5 is count deleted?) 
        # Actually for count=exact spread across deleted, it might be weird.
        # Let's try to parse it. If not present, return 0.
        if "/" in cr:
            try:
                total = cr.split("/", 1)[1].strip()
